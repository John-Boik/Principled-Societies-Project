...
def getFit(X, stocksDic, Print=False, Optimize=False):
    ...
    Note that G.nodes[src][dst][0] corresponds to dollar edges and ...[1] corresponds to token edges. 
    """
    ...
    if False:
        print("\nRead graph G")
        for node in G.nodes:
            print("\nnode={}, keys={}".format(node, G.nodes[node].keys()))
    ...
    personNodes = set(node for node in G.nodes if G.nodes[node]['kind']=='person')
    ...
    for p in personNodes:
        if G.nodes[p]['member'] == 0:
            G.nodes[p]['count'] = stocksDic[personStatusDic[p]]['cntNonLedda'] 
            G.nodes[p]['postCBFS_income_mean'] = stocksDic[personStatusDic[p]]['meanNonLedda']
            G.nodes[p]['postCBFS_income_total'] = stocksDic[personStatusDic[p]]['sumNonLedda']
        else:
            G.nodes[p]['count'] = stocksDic[personStatusDic[p]]['cntLedda']
            G.nodes[p]['postCBFS_income_mean'] = stocksDic[personStatusDic[p]]['meanLedda']
            assert wage_target == G.nodes[p]['postCBFS_income_mean']
            G.nodes[p]['postCBFS_income_total'] = stocksDic[personStatusDic[p]]['sumLedda']    
        #print("\n", personStatusDic[p], p, "    ", G.nodes[p])
    #print("\n")
    

    # Nodes    
    nodes = set(node for node in G.nodes)
    ...
    dollars= G.nodes['person_nonmember_NIWF']['postCBFS_income_total']
    G.edges[('gov', 'person_nonmember_NIWF', 0)]['value'] = np.maximum(0, dollars)
    ...
    cnt = G.nodes['person_member_NIWF']['count']
    dollars = G.nodes['person_nonmember_NIWF']['postCBFS_income_mean'] * cnt
    G.edges[('gov', 'person_member_NIWF', 0)]['value'] = np.maximum(0, dollars)
    ...
    G.edges[('CBFS_nurture', 'person_member_NIWF', 0)]['value'] = np.maximum(0, dollars)
    G.edges[('CBFS_nurture', 'person_member_NIWF', 1)]['value'] = np.maximum(0, tokens)
    

    # ---------- Unemployed, nonmember [2]
    dollars = G.nodes['person_nonmember_unemployed']['postCBFS_income_total']
    G.edges[('gov', 'person_nonmember_unemployed',0)]['value'] = np.maximum(0, dollars)
    
    
    # ---------- unemployed, member [3]                                                                                                                
    # gvt support for unem LEDDA pays portion of dollar requirement, nurture the rest
    cnt = G.nodes['person_member_unemployed']['count']
    dollars = G.nodes['person_nonmember_unemployed']['postCBFS_income_mean'] * cnt
    G.edges[('gov', 'person_member_unemployed',0)]['value'] = np.maximum(0, dollars)
    
    # CBFS, unemployed member
    dollars = (wage_target * cnt    * (1.-X.TSI) ) / (1 - earmarkSum)    - dollars
    tokens = wage_target * X.TSI * cnt / (1 - earmarkSum)
    G.edges[('CBFS_nurture', 'person_member_unemployed',0)]['value'] = np.maximum(0, dollars)
    G.edges[('CBFS_nurture', 'person_member_unemployed',1)]['value'] = np.maximum(0, tokens) 
    

    # ----------nonmember NP, nonmember person [4]
    dollars = G.nodes['person_nonmember_NP']['postCBFS_income_total']
    G.edges[('org_nonmember_NP', 'person_nonmember_NP',0)]['value'] = np.maximum(0, dollars)
    
    
    # ---------- nonmember SB, nonmember person [5]
    dollars = G.nodes['person_nonmember_SB']['postCBFS_income_total']
    G.edges[('org_nonmember_SB', 'person_nonmember_SB',0)]['value'] = np.maximum(0, dollars)


    # ----------member NP, member person [6]
    cnt = G.nodes['person_member_NP']['count']
    dollars = wage_target * (1.-X.TSI) * cnt / (1 - earmarkSum)
    tokens = wage_target * X.TSI * cnt / (1 - earmarkSum)
    G.edges[('org_member_NP', 'person_member_NP',0)]['value'] = np.maximum(0, dollars)
    G.edges[('org_member_NP', 'person_member_NP',1)]['value'] = np.maximum(0, tokens)    

                                                                                                                
    # ----------member SB, member person [7]
    cnt = G.nodes['person_member_SB']['count']
    dollars = wage_target * (1.-X.TSI) * cnt / (1 - earmarkSum)
    tokens = wage_target * X.TSI * cnt / (1 - earmarkSum)
    G.edges[('org_member_SB', 'person_member_SB',0)]['value'] = np.maximum(0, dollars)
    G.edges[('org_member_SB', 'person_member_SB',1)]['value'] = np.maximum(0, tokens)    
    

    # ---------- member PB, member person [8]
    cnt = G.nodes['person_member_PB']['count']
    dollars = wage_target * (1.-X.TSI) * cnt / (1 - earmarkSum)
    tokens = wage_target * X.TSI * cnt / (1 - earmarkSum)
    G.edges[('org_member_PB', 'person_member_PB',0)]['value'] = np.maximum(0, dollars)
    G.edges[('org_member_PB', 'person_member_PB',1)]['value'] = np.maximum(0, tokens)    
    

    # verify TSI
    for node in nodes:
        if (G.nodes[node]['kind'] == 'person') and (G.nodes[node]['member']):
            dollars_, tokens_, total_ = sumEdges(G, node, ['in'])    
            if np.isnan(tokens_):
                print("nan in verify TSI: node={:}, G[node]={:}, TSI={:}\n    d,t,total= {:}, {:}, {:}".format(
                    node, G.nodes[node], X.TSI, dollars_, tokens_, total_))
    ...
    wage_fractions_NP = np.array([
        G.nodes['person_nonmember_NP']['postCBFS_income_total'],
        G.nodes['person_member_NP']['postCBFS_income_total']
        ])
    ...
    for pn in personNodes:
        ...
        """
        assert G.edges[(pn, 'org_nonmember_NP',1)]['kind'] == 'regular_donation'
        G.edges[(pn, 'org_nonmember_NP',1)]['value'] = np.maximum(0, dollars * wage_fractions_NP[0] * \
            X.Config.NP_reg_donation_pct_dollar_income)        
        if G.nodes[pn]['member']:
            # spends tokens and dollars, plus donation dollars for three edges
            assert G.edges[(pn, 'org_member_NP', 2)]['kind'] == 'regular_donation'        
            G.edges[(pn, 'org_member_NP', 2)]['value'] = np.maximum(0, dollars * wage_fractions_NP[1] * \
                X.Config.NP_reg_donation_pct_dollar_income)
        else:
            assert G.edges[(pn, 'org_member_NP',1)]['kind'] == 'regular_donation' 
            G.edges[(pn, 'org_member_NP',1)]['value'] = np.maximum(0, dollars * wage_fractions_NP[1] * \
                X.Config.NP_reg_donation_pct_dollar_income)
        ...
        if G.nodes[pn]['member']:
            ...
            G.edges[(pn, 'CBFS_SB_subsidy',0)]['value'] = np.maximum(0, dollars)
            G.edges[(pn, 'CBFS_SB_subsidy',1)]['value'] = np.maximum(0, tokens)
            ...
            G.edges[(pn, 'CBFS_PB_subsidy',0)]['value'] = np.maximum(0, dollars)
            G.edges[(pn, 'CBFS_PB_subsidy',1)]['value'] = np.maximum(0, tokens)            
            ...
            G.edges[(pn, 'CBFS_NP_donation',0)]['value'] = np.maximum(0, dollars)
            G.edges[(pn, 'CBFS_NP_donation',1)]['value'] = np.maximum(0, tokens)     
            ...
            G.edges[(pn, 'CBFS_nurture',0)]['value'] = np.maximum(0, dollars)
            G.edges[(pn, 'CBFS_nurture',1)]['value'] = np.maximum(0, tokens)     
        ...
        for org in ['org_member_NP', 'org_member_PB', 'org_member_SB', 'org_nonmember_NP', 
            ...
            if not G.nodes[pn]['member']:
                ...
                G.edges[(pn, org,0)]['value'] = np.maximum(0, dollars_)
                ...
                if 'nonmember' in org:
                    ...
                    G.edges[(pn, org,0)]['value'] =    np.maximum(0, dollars_)
                    G.edges[(pn, org,1)]['value'] =    np.maximum(0, tokens_)                 
    ...
    wage_fractions_FP = np.array([
        G.nodes['person_nonmember_SB']['postCBFS_income_total'],
        G.nodes['person_member_SB']['postCBFS_income_total'],
        G.nodes['person_member_PB']['postCBFS_income_total']
        ])
    ...
    G.edges[('gov', 'org_nonmember_NP',0)]['value'] = NP_gov_spending * wage_fractions_NP[0]
    G.edges[('gov', 'org_member_NP',0)]['value']        = NP_gov_spending * wage_fractions_NP[1]
    
    G.edges[('gov', 'org_nonmember_SB',0)]['value'] = FP_gov_spending * wage_fractions_FP[0]
    G.edges[('gov', 'org_member_SB',0)]['value']        = FP_gov_spending * wage_fractions_FP[1]
    G.edges[('gov', 'org_member_PB',0)]['value']        = FP_gov_spending * wage_fractions_FP[2]
    ...
    assert tokens == 0
    dollars = np.maximum(0, dollars)
    G.edges[('gov', 'roc',0)]['value'] = dollars
    ...
    G.edges[('CBFS_SB_subsidy', 'org_member_SB',0)]['value'] = dollars
    G.edges[('CBFS_SB_subsidy', 'org_member_SB',1)]['value'] = tokens
    ...
    G.edges[('CBFS_PB_subsidy', 'org_member_PB',0)]['value'] = dollars
    G.edges[('CBFS_PB_subsidy', 'org_member_PB',1)]['value'] = tokens    
    ...
    G.edges[('CBFS_NP_donation', 'org_member_NP',0)]['value'] = dollars
    G.edges[('CBFS_NP_donation', 'org_member_NP',1)]['value'] = tokens    
    ...
    for org in ['org_member_NP', 'org_member_PB', 'org_member_SB', 'org_nonmember_NP', 
        ...
        if dollars_ < 0:
            ...
            G.edges[('roc', org,0)]['value'] = available
...
def getSums(G, edgeList, Print=False):
    ...
    for src, dst in edgeList:
        for ii in range(3):
            # not all edges have (node1, node2, 2)
            if (src, dst, ii) in G.edges:
                if G.edges[(src, dst, ii)]['unit'] == 'dollars':
                    dollars += G.edges[(src, dst, ii)]['value']
                else:
                    assert G.edges[(src, dst, ii)]['unit'] == 'tokens'
                    tokens += G.edges[(src, dst, ii)]['value']
...
def getTax(G, node, total, taxRate, deduction):
    ...
    Calculate taxes due
    """
    agi = total - (deduction * G.nodes[node]['count'])
    
    # deduct donation to NP, and CBFS contributions for NP and nurture
    agi -= G.edges[(node, 'org_nonmember_NP',0)]['value']
    agi -= G.edges[(node, 'org_member_NP',0)]['value']
    
    if G.nodes[node]['member']:
        agi -= G.edges[(node, 'CBFS_NP_donation',0)]['value']
        agi -= G.edges[(node, 'CBFS_NP_donation',1)]['value']
        agi -= G.edges[(node, 'CBFS_nurture',0)]['value']
        agi -= G.edges[(node, 'CBFS_nurture',1)]['value']        
...
def calc_fitness(G, nodes, Optimize, Print, delta_CBFS_ratio):
    ...
    for src in nodes:
        for dst in nodes:
            ...
            for ii in range(3):
                if (src, dst, ii) not in G.edges:  continue 
                
                edge = G.edges[(src, dst, ii)]
                ID = edge['id']
                edgeDic[ID] = edge 
        ...
        if not Optimize and (G.nodes[node]['kind'] == 'person'):
            ...
            if G.nodes[node]['member']:
                ...
                for e0, e1 in outEdges:
                    for ii in range(3):
                            # (e0,e1,2) might not exist
                            if (e0, e1, ii) not in G.edges:    continue
                            
                            if G.edges[(e0, e1, ii)]['kind'] in ["contribution"]:
                                    if G.edges[(e0, e1, ii)]['unit'] == 'dollars':
                                        CBFS_dollars += G.edges[(e0, e1, ii)]['value']
                                    else:
                                        assert G.edges[(e0, e1, ii)]['unit'] == 'tokens'
                                        CBFS_tokens += G.edges[(e0, e1, ii)]['value']        
                    
            cnt = G.nodes[node]['count']
            nodeDic[ID].update({
            ...
            expected_postCBFS = int(round(G.nodes[node]['postCBFS_income_total']))
            actual_postCBFS_mean = int(round(nodeDic[ID]['actual_postCBFS_income_total_mean']))
            expected_postCBFS_mean = int(round(G.nodes[node]['postCBFS_income_mean']))
    ...
    for node in nodes:
        ...
        for flow in ['in', 'out']:
            ...
            for e0, e1 in Edges:
                ...
                for ii in range(3):
                        # not all (e0,e1,2) exist
                        if (e0, e1,ii) not in G.edges:    continue
                                    
                        kind = G.edges[(e0, e1, ii)]['kind']
                        if kind not in d[flow][label].keys():
                                d[flow][label][kind] = [0,0]

                        if G.edges[(e0, e1, ii)]['unit'] == 'dollars':
                                d[flow][label][kind][0] = G.edges[(e0, e1, ii)]['value'] / divs
                        else:
                                assert G.edges[(e0, e1, ii)]['unit'] == 'tokens'
                                d[flow][label][kind][1] = G.edges[(e0, e1, ii)]['value'] / divs
